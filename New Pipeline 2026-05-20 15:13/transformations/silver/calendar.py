...
@dp.materialized_view(
    ...
    comment="Calendar Dimension with comprehensive date attributes and Indian Holidays (2020-2030)",
    table_properties={
        "qualtity":"transportation.silver.calendar",
        "layer":"silver",
...
def calendar():
    ...
    df = df.withColumn(
        ...
        .when(
            (F.col("month")==8) & (F.col("day_of_month")==15), F.lit("Independence Day")
        )
        .when(
            (F.col("month")==10) & (F.col("day_of_month")==2), F.lit("Gandhi Jayanti")
        )
        .when(
